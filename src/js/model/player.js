...
export default class Player
{
    ...
    renderControlsHolder()
    {
       ...
       <button class="liveStatus" style="display:none" id="liveStatus"> LIVE </button>
                ...
                <button id="settings"><svg class="create_icon  bg">
                            ...
                            <div id="hdcontainer"><span class="hd">Auto</span></div>
    ...
    showQualityList(e)
    {
        ...
        document.querySelector(".value_selector").innerHTML = getQualityList(this.qualityList, this.selectedQualityIndex, this.isLive);
    ...
    changeValues(e)
    {
        ...
        {
            this.selectedQualityIndex = parseInt(document.querySelector("input[type='radio'][name='quality_radio']:checked").value);

            this.setQuality(this.selectedQualityIndex);
            if (this.selectedQualityIndex == -1)
            {
                document.getElementById('hdcontainer').innerHTML = '<span class="hd">Auto</span>';
                return;
            } else if (this.qualityList[this.selectedQualityIndex].height >= 720)
            {
                document.getElementById('hdcontainer').innerHTML = `<span class="hd">Hd</span>`;
                return
            }
            document.getElementById('hdcontainer').innerHTML = ``;
    ...
    {
        if (this.isLive) return window.hls.nextLevel = parseInt(QualityIndex);
        window.hls.loadLevel = parseInt(QualityIndex);
    }

    setQualityList(list)
    {
        this.qualityList = list;
        if (this.isLive) this.setQualityForLive(list)

    }

    setQualityForLive(list)
    {
        window.hls.nextLevel = -1 //parseInt(list.length / 2)
        this.selectedQualityIndex = -1;
