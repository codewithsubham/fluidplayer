...
{	
	...
	if(conf.link.dash)
	{
		...
		player.setBufferTimeAtTopQualityLongForm(6*100);
		player.setBufferToKeep(600);
		player.setStableBufferTime(600);
		player.flushBufferAtTrackSwitch = true;
		player.bufferPruningInterval = 6000;
		player.bandwidthSafetyFactor = 0.1;
		//player.setFastSwitchEnabled(true);
		//player.pruneAllSafely;
		...
		player.enableLastMediaSettingsCaching(false);
...
{	
	...
	let bufferProgress = document.getElementById("bufferProgress"); //progress;
