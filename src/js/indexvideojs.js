...
{	
	...
	{	
		...
		//	player.on(Hls.Events.ERROR, onHlsError);
		 player = videojs('videoPlayer' , {children:false , html5: {
			hls: {
				 overrideNative: true,
			...
			document.querySelector('source').type = "application/x-mpegURL";
		...
		player.on('progress' , () => {
			onHlsFragmentLoad()
		console.log(console.log(player.tech_.hls.representations()));
		document.querySelector('#btnBigPlay').style.display = 'none';
...
{	
	...
	return new Promise((resolve , reject) => {
			...
			xmlhttp.open("GET","http://visionitlabs.com/vplayer3/bookmarks/getbookmarks.php?url="+videoURL+"&username="+conf.userName,true);
			xmlhttp.send();
...
function onBookmarkAddClick()
{
	...
	return new Promise((resolve , reject) => {
		...
		xmlhttp.open("GET","http://visionitlabs.com/vplayer3/bookmarks/addbookmark.php?url="+videoURL+"&comment="+document.getElementById("txtAddBookmark").value+"&timestamp="+video.currentTime+"&username="+conf.userName, true);
		xmlhttp.send();
...
function onBookmarkDeleteClick(val)
{
    ...
    xmlhttp.open("GET","http://visionitlabs.com/vplayer3/bookmarks/deletebookmark.php?id="+val+"&username="+conf.userName, true);
    xmlhttp.send();
