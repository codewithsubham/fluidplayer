...
function initPlayer(conf) {
    ...
    window.playbackSpeeds = [0.5, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0];
    window.playSpeed = 1;
    window.mute = 0;
    ...
    if (conf.controls.seekInterval) seekInterval = conf.controls.seekInterval;
    ...
    if (localStorage.getItem(`lastVideoDuration-${conf.videoId}`)) {
        video.currentTime = localStorage.getItem(`lastVideoDuration-${conf.videoId}`);
        //localStorage.clear();
    } else {
        video.currentTime = 0;
    }
    ...
    if (conf.link.hls) {
        ...
        if (Hls.isSupported()) {
            ...
            player.on(Hls.Events.FRAG_BUFFERED, onFragmentLoad);
            player.on(Hls.Events.ERROR, onHlsError);
            // player.on(Hls.Events.BUFFER_APPENDED, onHlsFragmentLoad);
            return;
        }
    }
    ...
    if (conf.link.dash) {
        ...
        player.on("error", function (e) {
            localStorage.setItem("lastVideoDuration-videoId", video.currentTime);
            player.reset();
            controls.remove();
            ErrorCode(
...
function onHlsError(event, data) {
    ...
    if (errorFatal) {
        if (errorCode.Hls[errorType][errorDetails]) {
            ErrorCode(
                errorCode.Hls[errorType][errorDetails],
                "btnBigPlay",
                "refreshVideo"
            );
        } else {
       ...
       // localStorage.setItem(`lastVideoDuration-${conf.videoId}`, video.currentTime);
        video.remove();
...
function ErrorCode(errorString, parent, classname) {
    ...
    //   document.querySelector(`.${parent}`).style.zIndex = 9;
}
function setPoster() {
...
function videoMetadataLoaded() {
    ...
    if (conf.controls != "false") initSeekbar();
    initProgress();
    ...
    document.getElementById("btnTimeDisplay").innerHTML =
        convertToTimecode(video.currentTime) + " / " + videoDurationText;
    if (conf.overlay) initOverlay();
}
...
function onFragmentLoad() {
    ...
    for (i = 0; i < video.buffered.length; i++) {
        if (video.buffered.end(i) >= seekbar.value) {
...
function addControlButton(
    ...
    if (class2 != "") elm.classList.add(class2);
    if (svg != "")
        elm.innerHTML = `<svg class="create_icon">
							<use xlink:href="img/svg/sprite.svg#${svg}"></use>
						</svg>`;

    if (listener) elm.addEventListener("click", listener);
    if (side == "left") {
        if (wrapper != "") {
...
function addControlButtonForMobile(
    id,
    class1,
    class2,
    label,
...
function drawControlBar() {
    ...
    if (conf.controls.seekBack != "false" && !mobileAndTabletcheck())
        addControlButton(
            ...
            "",
            "",
            "Seek " + seekInterval + " Sec Back",
            onBtnSeekBackClick,
            iconsName.FastBackward,
            "left"
        );
    if (conf.controls.seekFwd != "false" && !mobileAndTabletcheck())
        addControlButton(
            ...
            "",
            "",
            "Seek " + seekInterval + " Sec Forward",
            onBtnSeekFwdClick,
            iconsName.FastForward,
            "left"
        );
    ...
    if (conf.controls.mute != "false" && !mobileAndTabletcheck())
        addControlButton(
            ...
            "Mute",
            onBtnMuteClick,
            iconsName.VolumeHigh,
            "left",
            "volume-wrapper"
        );
    if (conf.controls.volSlider != "false") addVolumeSlider();
...
function drawLoader() {
    ...
    document.querySelector(`.${iconsName.CenterPlay}-wrapper`).style.display =
        "none";
    document.querySelector(`.${iconsName.CenterPause}-wrapper`).style.display =
        "none";
    // hide the seek button while video is being load;
    document.querySelector(
        ".icon-round-fast_rewind-24px-wrapper"
    ).style.visibility = "hidden";
    document.querySelector(
        ".icon-round-fast_forward-24px-wrapper"
    ).style.visibility = "hidden";
}
...
function drawBigPlayButton() {
    ...
    const icons = [
        iconsName.FastForward,
        iconsName.CenterPlay,
        "loader-container",
        iconsName.CenterPause,
        iconsName.FastBackward,
    ];
    icons.forEach((element) => {
        if (element == "loader-container") {
            ...
            center.insertAdjacentHTML(
                "afterbegin",
                `<button class='${element}-wrapper'><svg class="create_icon ${element} bg">
...
const clearLoader = () => {
    ...
    if (mobileAndTabletcheck()) {
        if (!videoIsBeingPlayed) {
            document.querySelector(
                `.${iconsName.CenterPlay}-wrapper`
            ).style.display = "flex";
            document.querySelector(
                `.${iconsName.CenterPause}-wrapper`
            ).style.display = "none";
        } else {
            document.querySelector(
                `.${iconsName.CenterPlay}-wrapper`
...
async function onBtnPipClick(e) {
    //TODO
...
function createBookmarkMenu() {
    ...
    return new Promise((resolve, reject) => {
        ...
        xmlhttp.open(
            "GET",
            "http://visionitlabs.com/bookmarks/getbookmarks.php?url=" +
                videoURL +
                "&username=" +
                conf.userName,
            true
...
async function createBookmarksList() {
    ...
    for (var i = 0; i < bookmarks.length; i++) {
        ...
        if (bookmarks[i].comment.length > bookmarkMenuWidth)
            bookmarkText =
                bookmarks[i].comment.slice(0, bookmarkMenuWidth) + "...";
        else bookmarkText = bookmarks[i].comment;
        ...
        btn.addEventListener(
            "click",
            onBookmarkDeleteClick.bind(this, bookmarks[i].bookmark_id),
            false
        );
        ...
        txt = document.createTextNode(
            convertToTimecode(bookmarks[i].location) + " - " + bookmarkText
        );
        ...
        if (bookmarks[i].type == 1) {
            recommendedBookmarkholder.children[1].insertAdjacentElement(
                "beforeend",
                elm
            );
        } else {
            userAddedBookmarkholder.children[1].insertAdjacentElement(
                "beforeend",
                elm
            );
        }
        bookmarktextcontainer.addEventListener(
            "click",
            onBookmarkItemClick.bind(this, bookmarks[i].location),
            false
        );
    }
...
function onBookmarkAddClick() {
    ...
    return new Promise((resolve, reject) => {
        ...
        xmlhttp.open(
            "GET",
            "http://visionitlabs.com/bookmarks/addbookmark.php?url=" +
                videoURL +
                "&comment=" +
                document.getElementById("txtAddBookmark").value +
                "&timestamp=" +
                video.currentTime +
                "&username=" +
                video.currentTime,
...
const addNewBookMark = async (data) => {
    ...
    else bookmarkText = boookmarklists.comment;
    ...
    btn.addEventListener(
        "click",
        onBookmarkDeleteClick.bind(this, boookmarklists.bookmark_id),
        false
    );
    btn.insertAdjacentHTML(
        "afterbegin",
    ...
    elm.appendChild(bookmarktextcontainer);
    txt = document.createTextNode(
        convertToTimecode(boookmarklists.location) + " - " + bookmarkText
    );
    bookmarktextcontainer.appendChild(txt);
    ...
    elm.classList.add("menu-item-bookmarks");
    document
        .querySelector(".userbookmark")
        .children[1].insertAdjacentElement("afterbegin", elm);
    bookmarktextcontainer.addEventListener(
        "click",
        onBookmarkItemClick.bind(this, boookmarklists.location),
        false
    );
...
function onBookmarkDeleteClick(val) {
    ...
    xmlhttp.open(
        "GET",
        "http://visionitlabs.com/bookmarks/deletebookmark.php?id=" +
            val +
            "&username=" +
            conf.userName,
        true
...
function onBookmarkItemClick(val) {
    if (event.target.id == "btnBookmarkDelete") return;
...
function onSeeking() {
    ...
    if (timeUpdateOnSeekUpdate == 0) {
        document.getElementById("btnTimeDisplay").innerHTML =
            convertToTimecode(curTime - videoDuration * remainingTimeDisplay) +
            " / " +
            videoDurationText;
        ...
        document.getElementById("btnTimeDisplay").innerHTML =
            convertToTimecode(timeUpdateOnSeekUpdate) +
            " / " +
            videoDurationText;
...
function onSeekbarMouseMove(event) {
    ...
    if (
        pos - tooltipwidth / 2 <= w - tooltipwidth &&
        pos - tooltipwidth / 2 >= 0
    ) {
    ...
    if (seeking)
        tooltip.innerHTML = convertToTimecode(
            curTime - videoDuration * remainingTimeDisplay
        );
    else tooltip.innerHTML = convertToTimecode((videoDuration / w) * pos);
...
function onPlaybackTimeUpdate() {
    ...
    videoDurationText = videoDurationText ? videoDurationText : "00:00:00";
    document.getElementById("btnTimeDisplay").innerHTML =
        convertToTimecode(curTime - videoDuration * remainingTimeDisplay) +
        " / " +
        videoDurationText;
...
function onBtnQualitySelectorClick() {
    ...
    if (
        document
            .getElementById("qualitySelectorMenu")
...
function createQualityMenu() {
    ...
    //console.log(availableQuality.length ,  "checking quality list");
...
function onQualityItemClick(index) {
    for (var i = -1; i < availableQuality.length; i++) {
        document
            .getElementById("video-quality-index-" + i)
            .classList.remove("menu-item-selected");
        document
            .getElementById("video-quality-index-" + i)
            .classList.add("menu-item-unselected");
    }
    if (index == -1) {
        if (urlType == "hls") {
            player.loadLevel = -1;
    ...
    document
        .getElementById("video-quality-index-" + index)
...
function onDocumentClick() {
    if (
        event.target.id == "btnSpeedSelector" ||
        event.target.id == "btnQualitySelector"
    )
        return;
    hideQualityMenu();
    hidePlaybackSpeedMenu();
...
function onBtnSpeedSelectorClick() {
    ...
    if (
        document.getElementById("playbackSpeedMenu").classList.contains("hide")
    ) {
...
function createPlaybackSpeedMenu() {
    ...
    document.getElementById("playbackSpeedMenu").insertAdjacentHTML(
        "afterbegin",
        `<div class='setting-header'><svg class="create_icon">
										<use xlink:href="img/svg/sprite.svg#icon-rewind"></use>
    ...
    for (var i = 0; i < playbackSpeeds.length; i++) {
        ...
        elm.addEventListener(
            "click",
            onPlaybackSpeedItemClick.bind(this, i),
            true
        );
    }
}
function onPlaybackSpeedItemClick(index) {
    ...
    document.querySelector(".icon-meter").innerHTML =
...
function onFullScreenChange() {
    if (isFullscreen()) {
        document.querySelector(
            "#btnFullscreen"
        ).innerHTML = `<svg class="create_icon">
...
function stickyToggle() {
    ...
    document.querySelector(':root').style.setProperty('--primary-color', 'lightblue');

    if (!stickyToggleStatus) {
        document
            .querySelector(".stickyToggleCheckbox")
            .setAttribute("checked", true);
        const status = (document.querySelector(
            ".stickyToggleCheckbox"
        ).value = true);
...
function onVideoPlay() {
    ...
    if (overlayDisplayTimeout == 0) drawTextOverlay(true);
    else drawTextOverlay(false);
...
function AvoidScreenForOverlay(percentage) {
    ...
    return (
        Math.floor(
            Math.random() * (bottomToBeAvoid - topToBeAvoid + 1) + topToBeAvoid
        ) - elm.offsetHeight
    );
}
function initOverlay() {
    window.overlayText = conf.overlay.overlayText
        ? conf.overlay.overlayText
        : "Text Overlay";
    window.overlayPosX = conf.overlay.posX
        ? conf.overlay.posX
        : "Math.random()*(container.offsetWidth - elm.offsetWidth)";
    window.overlayPosY = conf.overlay.posY
        ? conf.overlay.posY
        : "Math.random()* (container.offsetHeight - elm.offsetHeight) - 90";
    window.overlayDisplayDuration = conf.overlay.displayDuration
        ? "Math.random()*conf.overlay.displayDuration + 100"
        : "Math.random()*5000";
    window.overlayDisplayTimeout = conf.overlay.displayTimeout
        ? "Math.random()*conf.overlay.displayTimeout + 100"
        : "Math.random()*5000 + 100"; //+100 to avoid 0
}
function drawTextOverlay(permanent) {
    if (conf.overlay.show == false) return;
    if (!overlayDrawn) {
        ...
        elm.style.top = isFullscreen()
            ? AvoidScreenForOverlay(conf.overlay.avoidVerticalArea)
            : eval(overlayPosY);
        ...
        if (!permanent) {
            ...
            if (displayDuration < conf.overlay.minDisplayDuration) {
                window.setTimeout(
                    deleteTextOverlay,
                    conf.overlay.minDisplayDuration
                );
                return;
            }
...
function drawMobileNumberOverlay() {
    ...
    elm.style[conf.overlayPhoneNumber.positionX] = eval(
        conf.overlayPhoneNumber.posX
    );
    elm.style[conf.overlayPhoneNumber.positionY] = eval(
        conf.overlayPhoneNumber.posY
    );
}
